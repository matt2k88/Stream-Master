...
- **FFmpeg Audio Decoders (Android, v1.3.0+)**: Single-engine player on native is now `react-native-video` v6 (web still uses `expo-video`). The dual Player 1 / Player 2 selector and `PlayerEngineContext` were removed because the wrapper layer wasn't the cause of silent AC3/EAC3 audio on Firestick — the cause is missing licensed hardware decoders. Fix: `plugins/withFfmpegAudioRenderer.js` config plugin runs every prebuild and (a) patches `node_modules/react-native-video/.../ReactExoplayerView.java` to flip `EXTENSION_RENDERER_MODE_OFF` → `EXTENSION_RENDERER_MODE_PREFER` on the bundled `DefaultRenderersFactory`, and (b) injects `io.github.anilbeesetti:nextlib-media3ext:1.8.0-0.9.0` (community-prebuilt Media3 FFmpeg extension AAR — published at the official `androidx.media3.decoder.ffmpeg.*` class paths so `DefaultRenderersFactory` finds it via reflection without extra wiring). Result: ExoPlayer prefers FFmpeg software decoders over MediaCodec for audio tracks (AC3 / EAC3 / DTS / MP2), so Firestick devices that lack licensed hardware decoders now produce sound. Video still goes through MediaCodec hardware. APK +~10MB. Plugin throws build-time errors (not warnings) if either patch site is missing — fail-loud is intentional so a broken APK isn't shipped silently. **Version coupling**: nextlib's version string `1.8.0-0.9.0` MUST match `RNVideo_media3Version` in `node_modules/react-native-video/android/gradle.properties`. If `react-native-video` is bumped and Media3 moves, look up the matching nextlib release at https://central.sonatype.com/artifact/io.github.anilbeesetti/nextlib-media3ext and update `NEXTLIB_VERSION` in the plugin.
