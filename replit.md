...
- **Music Player (Spotify-style)**: Header `music` button → `MusicHomeScreen` with curated rows (Top Pop / Hip-Hop Heat / R&B / Rock / Country / Latin / Electronic / Throwbacks) sourced from iTunes Search. `MusicSearchScreen` queries iTunes by song/artist. Playback flow: server resolves the iTunes track to a YouTube videoId (scrape of YouTube results page, cached in Supabase `music_resolved_tracks`), then `yt-dlp` extracts a direct audio stream URL (cached in-memory ~5h, refreshed on player error). Audio plays via `expo-audio` (`useAudioPlayer`) — no WebView, no YouTube IFrame, no Vevo/Topic embed-block errors. If extraction fails for all candidates, falls back to the iTunes 30-second preview clip. `MusicHost` is mounted once at the App root inside `MusicProvider` so audio survives every navigation. A persistent mini-bar (artwork, title, prev/play/next, progress) is pinned to the bottom whenever a track is loaded — tap to open `NowPlayingScreen` (large artwork, scrub bar, 15s skip, full controls). Playlists are per-profile, stored in Supabase (`music_playlists` + `music_playlist_tracks`). `MusicPlaylistsScreen` lists them with cover art / song count; `MusicPlaylistDetailScreen` plays from any track, long-press a row to remove. `AddToPlaylistModal` is invoked from search results to add a track (create-new-playlist inline supported). Tapping a track auto-builds a queue from the surrounding list so next/prev work naturally.

  **yt-dlp setup**: Requires Python yt-dlp ≥ 2026.03.17 (the Nix 2024.05.27 build is too old — signature extraction fails). Install via `uv` to `.pythonlibs/`. Server spawns `python -m yt_dlp -f 'bestaudio[ext=m4a]/bestaudio' -g <url>` with `PYTHONPATH` pointing at `.pythonlibs/lib/python3.11/site-packages` so the new version is loaded, not the stale Nix one. Caveats: Replit IP may be rate-limited by YouTube; extracted URLs expire ~6h and are IP-bound (client receives a fresh URL via `/api/music/audio?refresh=1` on player error); first-play latency 3–8 s while yt-dlp runs.
