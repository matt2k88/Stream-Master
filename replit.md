...
- **PlayerScreen Seek Keys**: Focusing the seek bar immediately enables arrow-key seeking — no separate "tap to capture" step. Web/desktop ArrowLeft/ArrowRight seek ±10s; TV remote left/right seek with hold-to-accelerate (5/10/20/40/60s steps based on hold duration). Up/Down on the D-pad moves focus to the surrounding controls so the user can leave the bar.
