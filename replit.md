...
- `migrations/016_football_fixture_channels.sql` — creates `football_fixture_channels` table (id pk, fixture_id bigint FK→football_fixtures, channel_name text, stream_id text nullable, created_at). Maps a fixture to the live TV channel(s) broadcasting it (populated externally). Read via `GET /api/football/centre/channels` (optional `?fixture_ids=` filter); the Football Centre renders a tappable channel badge that deep-links to the live channel. Until run, the endpoint returns `[]` gracefully.
