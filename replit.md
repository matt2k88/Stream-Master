...
- **PlayerScreen Seek Keys**: Focusing the seek bar immediately enables arrow-key seeking — no separate "tap to capture" step. Web/desktop ArrowLeft/ArrowRight seek ±10s; TV remote left/right seek with hold-to-accelerate (5/10/20/40/60s steps based on hold duration). Pressing OK on the focused bar toggles a "released" mode (icon flips from move to skip-forward) so D-pad left/right can move past the seek bar to the CC and Audio buttons; pressing OK again re-arms seeking. Up/Down on the D-pad moves focus normally to other rows. Re-focusing the bar re-arms seeking automatically.
