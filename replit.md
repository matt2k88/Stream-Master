...
- **Music Player (Spotify-style)**: Header `music` button → `MusicHomeScreen` with curated rows (Top Pop / Hip-Hop Heat / R&B / Rock / Country / Latin / Electronic / Throwbacks) sourced from iTunes Search. `MusicSearchScreen` queries iTunes by song/artist. Playback resolves the iTunes track to a YouTube video id at play time (server-side scrape of the YouTube results page, no API key, cached forever in Supabase `music_resolved_tracks`). The server then extracts a direct googlevideo CDN audio URL via yt-dlp (cookies + yt-dlp-ejs Deno plugin for the JS challenge) and proxies the bytes to the client through `/api/music/audio/:videoId` (range-aware). The client plays it with **`expo-audio`** (`useAudioPlayer`) mounted at the App root in `MusicHost` — audio-only, no Surface required, works consistently across iOS / Android / web (the earlier `expo-video` + hidden 1×1 `VideoView` approach was flaky on Android ExoPlayer). A persistent mini-bar (track art, title, prev/play/next, progress) is pinned to the bottom whenever a track is loaded — tap to open `NowPlayingScreen` (large artwork, scrub bar, 15s skip, full controls). **Web autoplay unlock**: `MusicContext.startTrackAt` calls `controllerRef.current?.play()` synchronously inside the tap handler *before* the async resolve fetch, so the browser still has a live user gesture by the time the real source is loaded (otherwise Chrome blocks the post-fetch play() and audio would only start after the user pressed back or interacted again). The mini-bar shows a longer hint ("first track can take a few seconds…") while loading the first track of a session so the brief yt-dlp cold-start doesn't feel like a freeze. Playlists are per-profile, stored in Supabase (`music_playlists` + `music_playlist_tracks`). `MusicPlaylistsScreen` lists them with cover art / song count; `MusicPlaylistDetailScreen` plays from any track, long-press a row to remove. `AddToPlaylistModal` is invoked from search results to add a track (create-new-playlist inline supported). Tapping a track auto-builds a queue from the surrounding list (curated row, search results, or playlist) so next/prev work naturally.
...
- `migrations/012_music_playlists.sql` — creates `music_playlists` (per-profile playlists), `music_playlist_tracks` (ordered iTunes-track snapshots inside a playlist), and `music_resolved_tracks` (global iTunes-track-id → YouTube-video-id cache so each song is resolved at most once across all users).
