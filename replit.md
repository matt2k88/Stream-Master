...
- **Selectable Video Player (Default / VLC)**: Account screen → utility block → "Player: Default | VLC" toggle (with confirm alert). Persisted in AsyncStorage (`ultracast.player.engine.v1`) via `PlayerEngineContext`. **Default** uses expo-video (the original player with full PlayerScreen UI: continue watching, next-episode prompt, favourite star, report content, D-pad seek bar, hold-to-accelerate skip, CC/audio panels, etc.). **VLC** routes both `Player` and `LivePreview` routes through `VlcPlayerScreen` instead — a slimmer fullscreen UI that uses libVLC for playback (much wider codec coverage: Dolby AC3/EAC3, DTS, MP2, raw MPEG-TS, etc.). The VLC screen has play/pause, ±10s skip, audio/subtitle track selectors, mute, basic seek bar (read-only progress for now), recently_watched continue-watching save (every ~10s + completion at last 30s), and resume-from-saved-time. Switching engines is instant (no app restart needed). Powered by `react-native-vlc-media-player` (Expo plugin auto-applies the libc++_shared gradle workaround). VLC engine is native-only — falls back to a "switch back to Default" message on web.
