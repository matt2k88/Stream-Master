...
- **PlayerScreen Seek Keys**: Web/desktop — ArrowLeft/ArrowRight always seek ±10s whenever the player is active (not live, duration>0, no CC/Audio panel open, no report modal open); no focus requirement since the player is fullscreen. TV remote — left/right seek with hold-to-accelerate (5/10/20/40/60s steps) only when the seek bar has D-pad focus. Pressing OK on the focused bar toggles a "released" mode (icon flips) so D-pad can navigate to CC/Audio; re-focusing the bar re-arms seeking.
